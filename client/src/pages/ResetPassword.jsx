...
const ResetPassword = () => {
  ...
  const onSubmitEmail = async (e) => {
    ...
    try {
      ...
      axios.defaults.withCredentials = true; // Ensure cookies are sent with request
  ...
  return (
    <div className="flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to to-purple-400">
      ...
      {!isEmailSent && (
        <form onSubmit={(e) => onSubmitEmail(e)} className="bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm">
      ...
      {!isOtpSubmited && isEmailSent && (
        <form onSubmit={onSubmitOtp} className="bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm">
          ...
          <button className="w-full py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full" type="submit">
            Submit
          </button>
        </form>
      )}
